...
public class SipCall implements SipActionObject, MessageListener
{
	...
	protected SipCall(SipPhone phone, Address myAddress)
	{
		...
		allReceivedMessagesContent = Collections
	...
	/**
	 * Start listening for a MESSAGE request. This is a non-blocking call.
	 * Starting from the time this method is called, any received request(s) for
	 * this UA are collected. After calling this method, call waitForMESSAGE()
	 * to process the first MESSAGE received since calling this method.
	 * 
	 * @return true unless an error is encountered, in which case false is
	 *         returned.
	 */    
	...
	/**
	 * The waitForMessage() method waits for a MESSAGE request addressed to this user
	 * agent to be received from the network. Prior to calling this method, any
	 * of the listenForXyz() methods must have already been called to turn on
	 * request listening. 
	 * <p>
	 * This method blocks until one of the following occurs: 1) A MESSAGE request
	 * message has been received, addressed to this user agent. In this case, a
	 * value of true is returned. The getLastReceivedRequest() method can be
	 * called to get information about the received MESSAGE request. 2) The wait
	 * timeout period specified by the parameter to this method expires. false
	 * is returned in this case. 3) An error occurs. false is returned in this
	 * case.
	 * <p>
	 * Any non-MESSAGE requests received for this user agent are collected while
	 * waiting for an MESSAGE message and can be seen by calling
	 * getAllReceivedRequests() once this method returns.
	 * <p>
	 * Regardless of the outcome, incoming requests associated with this User
	 * Agent will continue to be queued up until the stopListeningForRequests()
	 ...
	 * @return false in the case of wait timeout or error; call getReturnCode()
	 *         and/or getErrorMessage() and, if applicable, getException() for
	 *         further diagnostics. A true value is returned if a MESSAGE message
	 *         was received. Call the getLastReceivedRequest() method to get
	...
	/**
	 * This method sends a basic response to a previously received MESSAGE
	 * request. The response is constructed based on the parameters passed in.
	 * Call this method after waitForMessage() returns true. Call this
	 * method multiple times to send multiple responses to the received MESSAGE.
	 * The MESSAGE being responded to must be the last received request on this
	 * SipCall.
	 * 
	 * @param statusCode
	...
	/**
	 * This method is the same as the basic sendMessageResponse() method
	...
	public boolean sendMessageResponse(int statusCode, String reasonPhrase, int expires, 
		...
		if (body!=null){
			...
			try {
				contentTypeHeader = parent.getHeaderFactory().createContentTypeHeader("text", "plain;charset=UTF-8");
		...
		if (parent.sendReply(transaction, statusCode, reasonPhrase, myTag,
				null, expires, additionalHeaders,
	 ...
	 * calling program must subsequently call the waitOutgoingMessageResponse()
	 * method (one or more times) to get the result(s).
	 * 
	 * If a DIALOG exists the method will use it to send the MESSAGE
	 * 
	 * @param toUri
	 *            The URI (sip:bob@nist.gov) to which the message should be
	 *            directed
	 * @param viaNonProxyRoute
	 *            Indicates whether to route the MESSAGE via Proxy or some other
	...
	/**
	 * This method is the same as the basic initiateOutgoingMessage() method except
	 * that it allows the caller to specify a message body.
	 * 
	 * If a DIALOG exists the method will use it to send the MESSAGE
	 * 
	 * The extra parameters supported by this method are:
	...
	/**
	 * This method is the same as the basic initiateOutgoingMessage() method except
	 * that it allows the caller to specify a from URI and/or additional
			...
			ArrayList<Header> additionalHeaders, ArrayList<Header> replaceHeaders, String body)
	{
		...
		try
		{
			...
			if (dialog == null){
				...
				FromHeader from_header = hdr_factory.createFromHeader(from_address,
						myTag);
				...
				msg = parent.getMessageFactory().createRequest(request_uri,
			...
			if (body != null){
				ContentTypeHeader contentTypeHeader = parent.getHeaderFactory().createContentTypeHeader("text", "plain;charset=UTF-8");            
			...
			// processEvent()
			{
				transaction = parent.sendRequestWithTransaction(msg, viaProxy,
						dialog, null, additionalHeaders, replaceHeaders,
						body);
			}
	...
	/**
	 * The waitOutgoingMessageResponse() method waits for a response to be received
	 * from the network for a sent MESSAGE. Call this method after calling
	 ...
	 * 
	 * @return A Request object representing the last request message
	 *         received on this call, or null if none has been received.
	 * 
	 */
