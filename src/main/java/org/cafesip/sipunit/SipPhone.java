...
import org.cafesip.sipunit.processing.notify.ConferenceEventStrategy;
import org.cafesip.sipunit.processing.notify.PresenceEventStrategy;
import org.cafesip.sipunit.processing.notify.ReferEventStrategy;
...
public class SipPhone extends SipSession implements SipActionObject, RequestListener {
  ...
  /**
   * Processes NOTIFY request for this user agents depending on the configured processing strategy. If no strategies
   * are matched to the request being processed, then the processed NOTIFY is deemed orphaned and this UA will return
   * {@link SipResponse#CALL_OR_TRANSACTION_DOES_NOT_EXIST}
   */
  private RequestProcessor<SipPhone> notifyProcessor = new RequestProcessor<>(
          // Test that we support the event specified in the event header
          new PresenceEventStrategy(), new ReferEventStrategy(), new ConferenceEventStrategy()
  );
  ...
  private void processRequestEvent(RequestEvent requestEvent) {
    boolean validationSuccessful = isNotifyRequest(requestEvent) && containsEventHeader(requestEvent);
    RequestProcessingResult result = new RequestProcessingResult(!validationSuccessful, false);

    // Validation passed, continue with event handling
    if (!result.isProcessed()) {
      result = notifyProcessor.processRequestEvent(requestEvent, this);
    }

    if (!result.isProcessed()) {
      handleUnknownEvent(requestEvent);
    } else if (!result.isSuccessful()) {
      handleOrphanedEvent(requestEvent);
    }
  }

  private boolean isNotifyRequest(RequestEvent requestEvent) {
    Request request = requestEvent.getRequest();

    if (!request.getMethod().equals(Request.NOTIFY)) {
      this.sendResponse(requestEvent, SipResponse.SERVER_INTERNAL_ERROR,
              "Expected to receive a NOTIFY request, but instead got: " + request.getMethod());

      String err = "*** NOTIFY REQUEST ERROR ***  (SipPhone " + me
              + ") - SipPhone.processRequestEvent() - incoming request was routed incorrectly, expected NOTIFY but got "
              + request.getMethod() + " : " + requestEvent;
      this.distributeEventError(err);
      LOG.error(err);
      return false;
    }
  ...
  private boolean containsEventHeader(RequestEvent requestEvent) {
    Request request = requestEvent.getRequest();

    EventHeader event = (EventHeader) request.getHeader(EventHeader.NAME);
    if (event == null) {
      this.sendResponse(requestEvent, SipResponse.BAD_REQUEST,
  ...
  private void handleUnknownEvent(RequestEvent requestEvent) {
    Request request = requestEvent.getRequest();
    EventHeader event = (EventHeader) request.getHeader(EventHeader.NAME);

    if (event != null) {
      String error = "Received a NOTIFY request with unrecognized event header : " + event.getEventType();
      this.sendResponse(requestEvent, SipResponse.BAD_EVENT, error);
  ...
  /**
   * We need to enforce a default handling method. Assuming there are no strategies left, we need to respond to the
   * case where we aren't able to do any NOTIFY processing
   */
  private void handleOrphanedEvent(RequestEvent requestEvent) {
    Request request = requestEvent.getRequest();
    ...
    this.sendResponse(requestEvent, SipResponse.CALL_OR_TRANSACTION_DOES_NOT_EXIST, err);

    String error = "*** NOTIFY REQUEST ERROR ***  (" + from.getAddress().getURI().toString()
            + ") : " + err + " : " + request.toString();
    this.distributeEventError(error);
    LOG.error(error);
  }

  /**
   * Common-base response sending function used in NOTIFY processing
   *
   * @param req    The request this response is being constructed for
   * @param status Status code
   * @param reason Reason string displayed along the status code
   */
  protected void sendResponse(RequestEvent req, int status, String reason) {
    try {
      Response response = getMessageFactory().createResponse(status, req.getRequest());
  ...
  /**
   * Sets the error state for the available subscriptions of the specified phone. The error is propagated
   * to all the subscriptions - test program will need to see the error.
   *
   * @param err The error message
   * @see SipAssert#assertNoSubscriptionErrors
   */
  private void distributeEventError(String err) {
    List<EventSubscriber> subscriptions = new ArrayList<EventSubscriber>(getBuddyList().values());
    ...
    for (EventSubscriber eventSubscriber : subscriptions) {
      eventSubscriber.addEventError(err);
    }
  }
  ...
  /**
   * @return The request processor for NOTIFY requests which are validated and accepted to for this instance. The
   * user may use this instance to add additional NOTIFY request processing strategies in order to provide extended
   * capability of handling event packages in SIP.
   */
  public RequestProcessor<SipPhone> getNotifyProcessor() {
