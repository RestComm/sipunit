...
public class TestNoProxyTLS
{
    ...
    // INVITE myself
    {
        try
        {
            ...
            Request invite = ua.getParent().getMessageFactory().createRequest(
                    "INVITE sips:becky@nist.gov SIP/2.0\r\n\r\n");
            ...
            Address to_address = addr_factory.createAddress(addr_factory
                    .createURI("sips:becky@nist.gov"));
            invite.addHeader(hdr_factory.createToHeader(to_address, null));

            Address contact_address = addr_factory.createAddress("sips:amit@"
                    + ua.getStackAddress() + ':'
                    + myPort);
            ...
            Address route_address = addr_factory.createAddress("sips:becky@"
    ...
    public void testRaceConditionRequestBeatsResponse()
    {
        ...
        try
        {
            ...
            a.initiateOutgoingCall("sips:amit@nist.gov", "sips:becky@nist.gov",
            		ua.getStackAddress() + ':'
                            + myPort + '/' + testProtocol,
    ...
    // string
    {
        try
        {
            ...
            a.initiateOutgoingCall("sips:becky@nist.gov", ua.getStackAddress()
                    + ':' + myPort + '/' + testProtocol);
            assertLastOperationSuccess("a initiate call - " + a.format(), a);
    ...
    public void testMakeCallFail()
    {
        try
        {
            ua.makeCall("sips:becky@nist.gov", SipResponse.RINGING, 1000,
            		ua.getStackAddress() + ':'
                            + myPort + '/' + testProtocol);
    ...
    // SipPhone.makeCall() - A CALLS B
    {
        try
        {
            ...
            SipCall a = ua.makeCall("sips:becky@nist.gov", ua.getStackAddress()
                    + ':' + myPort + '/' + testProtocol);
    ...
    // SipPhone.makeCall()
    {
        ...
        try
        {
            ...
            SipCall a = ua.makeCall("sips:becky@nist.gov", SipResponse.OK, 5000,
            		ua.getStackAddress() + ':'
                            + myPort + '/' + testProtocol);
            assertLastOperationSuccess(ua.format(), ua);
    ...
    // SipPhone.makeCall() with extra String parameters
    {
        ...
        try
        {
            ...
            replace_hdrs.add(new String("Contact: <sips:doodah@"
                    + ua.getStackAddress() + ':'
                    + myPort + '>'));
    ...
    public void testReceivingACKAfterCancel() throws Exception
    {
        ...
        assertRequestReceived(
                "sips: INVITE not received<br>(last received request is "
        ...
        assertLastOperationSuccess(
                "sips: 183 Session Progress is not correcly sent", callee);
        ...
        String lastRequest2 = (req2 == null) ? "no request received" : req2
                .getMessage().toString();
        assertRequestReceived(
                "sips: CANCEL is not received<br>(last received request is "
                        + lastRequest2 + ")", SipRequest.CANCEL, callee);

        callee.respondToCancel(calleeCancelTrans, SipResponse.OK, "OK", 0);
        assertLastOperationSuccess("sips: could not send 200 OK for the CANCEL",
                callee);
        Thread.sleep(200);
        ...
        callee
                .sendIncomingCallResponse(SipResponse.REQUEST_TERMINATED,
                        ...
                        null, null);
        assertLastOperationSuccess(
                "sips: could not send 487 Request Terminated send for the CANCEL",
                callee);
        Thread.sleep(500);
        ...
        lastRequest = (req3 == null) ? "no request received" : req3
                .getMessage().toString();
        assertRequestReceived(
                "sips: ACK not received<br>(last received request is "
