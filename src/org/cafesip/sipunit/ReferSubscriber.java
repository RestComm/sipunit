 ...
 * sending a REFER request via the SipPhone.refer() method. This object is used
 * by a test program to proceed through the initial REFER-NOTIFY and any
 * subsequent SUBSCRIBE-NOTIFY sequence(s) and to find out details at any given
 * time about the subscription such as the subscription state, amount of time
 * left on the subscription if still active, termination reason if terminated,
 * errors encountered during received REFER/SUBSCRIBE responses and incoming
 * NOTIFY message validation, and details of any received responses and requests
 * if needed by the test program.
 * <p>
...
public class ReferSubscriber extends SubscriptionSubscriber
{
    ...
    /**
     * Removes this object from the SipPhone referrer list. No check is done.
     * You should unsubscribe() before calling this method.
     */
    public void dispose()
    {
    ...
    protected AcceptHeader getUnsupportedMediaAcceptHeader()
            throws ParseException
    {
        return parent.getHeaderFactory().createAcceptHeader("message",
                "sipfrag");
    }

    /**
     * This method initiates a SUBSCRIBE/NOTIFY sequence for the purpose of
     * refreshing this subscription. This method creates a SUBSCRIBE request
     * message based on the parameters passed in, sends out the request, and
     * waits for a first response to be received. It saves the received response
     * and checks for a "proceedable" (positive) status code value. Positive
     * response status codes include any of the following: provisional (status /
     * 100 == 1), UNAUTHORIZED, PROXY_AUTHENTICATION_REQUIRED, OK and ACCEPTED.
     * Any other status code, or a response timeout or any other error, is
     * considered fatal to this refresh operation.
     * <p>
     ...
     *            refresh method call or on calling unsubscribe().
     * @param timeout
    ...
    /**
     * This method is the same as refresh(duration, eventId, timeout) except
     * that the SUBSCRIBE duration sent will be however much time is left on the
    ...
    /**
     * This method is the same as refresh(duration, eventId, timeout) except
     * that the eventId remains unchanged from whatever it already was.
     */
    ...
    /**
     * This method is the same as refresh(duration, eventId, timeout) except
     * that the eventId remains unchanged from whatever it already was and the
     * SUBSCRIBE duration sent will be however much time is left on the current
     * subscription. If time left on the subscription <= 0, unsubscribe occurs.
     */
    ...
    /**
     * This method is the same as refresh(duration, eventId, timeout) except
     * that instead of creating the SUBSCRIBE request from parameters passed in,
     * the given request message parameter is used for sending out the SUBSCRIBE
     * message.
     * <p>
     ...
     * unsubscribe. The event "id" in the given request will be used
     * subsequently (for error checking SUBSCRIBE responses and NOTIFYs from the
     * server as well as for sending subsequent SUBSCRIBEs).
     */
    ...
    /**
     * This method initiates a SUBSCRIBE/NOTIFY sequence to terminate the
     * subscription unless the subscription is already terminated.
     * <p>
     ...
     * sends out the request, and waits for a first response to be received. It
     * saves the received response and checks for a "proceedable" (positive)
     * status code value. Positive response status codes include any of the
     * following: provisional (status / 100 == 1), UNAUTHORIZED,
     ...
     *         needed, false otherwise. See more details above.
     */
    ...
    /**
     * This method is the same as unsubscribe(eventId, timeout) except that no
    ...
    /**
     * This method is the same as unsubscribe(eventId, timeout) except that
