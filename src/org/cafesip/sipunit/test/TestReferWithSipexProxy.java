 ...
 * that can pass received REFER requests/responses on to the target registered
 * User Agent endpoints, and for refresh/unsubscribe (via SUBSCRIBE) do the
 * same.
 * 
 * When using CafeSip SipExchange for this test, you need to modify a couple of
 * its default config settings to achieve the above. Open the sipexchange.ear in
 * your JBoss deploy directory and within that, open sipex-jiplets.spr (it is a
 * normal zip archive and can be opened with something like WinZip). Edit the
 * JIP-INF/jip.xml file in the sipex-jiplets.spr. Make these two changes:
 * 
 * <pre>
 ...
 * used here in the list of users/subscribers at the proxy, all with password
 * a1b2c3d4 - these URIs include: sip:becky@<property "sipunit.test.domain"
 * below>, sip:amit@<property "sipunit.test.domain" below>.
 * 
 * 
